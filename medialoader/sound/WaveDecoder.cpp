...
WaveDecoder::WaveDecoder(const char* data, size_t size, int bufferSize)
	: Decoder(data, size, bufferSize)
{
	dataFile.data = (char *) data;
	dataFile.size = size;
	dataFile.offset = 0;

	int wuff_status = wuff_open(&handle, &WaveDecoderCallbacks, &dataFile);
	if (wuff_status < 0)
		throw Exception("Could not open WAVE");

	try
	{
		wuff_status = wuff_stream_info(handle, &info);
		if (wuff_status < 0)
			throw Exception("Could not retrieve WAVE stream info");

		if (info.channels > 2)
			throw Exception("Multichannel audio not supported");

		if (info.format != WUFF_FORMAT_PCM_U8 && info.format != WUFF_FORMAT_PCM_S16)
		{
			wuff_status = wuff_format(handle, WUFF_FORMAT_PCM_S16);
			if (wuff_status < 0)
				throw Exception("Could not set output format");
		}
	}
	catch (Exception &)
	{
...
{
	return new WaveDecoder(data, size, bufferSize);
}
