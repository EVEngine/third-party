...
FLACDecoder::FLACDecoder(const char* data, size_t size, int nbufferSize)
: Decoder(data, size, nbufferSize)
{
	flac = drflac_open_memory(data, size, nullptr);
	if (flac == nullptr)
		throw Exception("Could not load FLAC file");
}
...
{
	return new FLACDecoder(data, size, bufferSize);
}
