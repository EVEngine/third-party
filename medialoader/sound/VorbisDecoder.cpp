...
{
	int64_t    spaceToEOF;   // How much more we can read till we hit the EOF marker
	int64_t    actualOffset; // How much we can actually offset it by
...
VorbisDecoder::VorbisDecoder(const char* data, size_t size, int bufferSize)
	...
	, duration(-2.0)
{
	...
	// Initialize OGG file
	oggFile.dataPtr = data;
	oggFile.dataSize = size;
	oggFile.dataRead = 0;

	// Open Vorbis handle
	if (ov_open_callbacks(&oggFile, &handle, NULL, 0, vorbisCallbacks) < 0)
		throw Exception("Could not read Ogg bitstream");
...
{
	return new VorbisDecoder(data, size, bufferSize);
}
