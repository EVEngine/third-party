...
ModPlugDecoder::ModPlugDecoder(const char* data, size_t size, int bufferSize)
	...
	, duration(-2.0)
{
	...
	// Load the module.
	plug = ModPlug_Load(data, (int) size);

	if (plug == 0)
		throw Exception("Could not load file with ModPlug.");
...
{
	return new ModPlugDecoder(data, size, bufferSize);
}
