...
int getBitsPerPixel(uint64_t pixelformat)
{
...
void getFormatMinDimensions(uint64_t pixelformat, int &minX, int &minY, int &minZ)
{
	minZ = 1;
...
bool PVRHandler::canParseCompressed(const char* data, size_t size)
{
	if (size < sizeof(PVRTexHeaderV2) || size < sizeof(PVRTexHeaderV3))
		return false;

	PVRTexHeaderV3 *header3 = (PVRTexHeaderV3 *) data;
	...
	PVRTexHeaderV2 *header2 = (PVRTexHeaderV2 *) data;
...
CompressedMemory* PVRHandler::parseCompressed(const char* data, size_t size, std::vector<CompressedSlice*> &images, PixelFormat &format, bool &sRGB)
{
	if (!canParseCompressed(data, size))
		throw Exception("Could not decode compressed data (not a PVR file?)");

	PVRTexHeaderV3 header3 = *(PVRTexHeaderV3 *) data;

	// If the header isn't the V3 format, assume it's V2 and convert.
	if (header3.version != PVRTEX3_IDENT && header3.version != PVRTEX3_IDENT_REV)
		ConvertPVRHeader(*(PVRTexHeaderV2 *) data, &header3);
	...
	if (header3.depth > 1)
		throw Exception("Image depths greater than 1 in PVR files are unsupported.");
	...
	if (cformat == PIXELFORMAT_UNKNOWN)
		throw Exception("Could not parse PVR file: unsupported image format.");
	...
	if (size < fileoffset + totalsize)
		throw Exception("Could not parse PVR file: invalid size calculation.");
