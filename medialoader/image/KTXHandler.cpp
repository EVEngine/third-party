...
bool KTXHandler::canParseCompressed(const char* data, size_t size)
{
	if (size < sizeof(KTXHeader))
		return false;

	KTXHeader *header = (KTXHeader *) data;
	uint8_t ktxidentifier[12] = KTX_IDENTIFIER_REF;
...
CompressedMemory* KTXHandler::parseCompressed(const char* data, size_t size, std::vector<CompressedSlice*> &images, PixelFormat &format, bool &sRGB)
{
	if (!canParseCompressed(data, size))
		throw Exception("Could not decode compressed data (not a KTX file?)");

	KTXHeader header = *(KTXHeader *) data;

	if (header.endianness == KTX_ENDIAN_REF_REV)
	{
		uint32_t *headerArray = (uint32_t *) &header.glType;
		for (int i = 0; i < 12; i++)
	...
	if (cformat == PIXELFORMAT_UNKNOWN)
		throw Exception("Unsupported image format in KTX file.");

	if (header.numberOfArrayElements > 0)
		throw Exception("Texture arrays in KTX files are not supported.");

	if (header.pixelDepth > 1)
		throw Exception("3D textures in KTX files are not supported.");

	if (header.numberOfFaces > 1)
		throw Exception("Cubemap textures in KTX files are not supported.");

	size_t fileoffset = sizeof(KTXHeader) + header.bytesOfKeyValueData;
	const uint8_t *filebytes = (uint8_t *) data;
	size_t totalsize = 0;

	// Calculate the total size needed to hold the data in memory.
	for (int i = 0; i < (int) header.numberOfMipmapLevels; i++)
	{
		if (fileoffset + sizeof(uint32_t) > size)
			throw Exception("Could not parse KTX file: unexpected EOF.");

		uint32_t mipsize = *(uint32_t *) (filebytes + fileoffset);
