...
bool DDSHandler::canDecode(const char* data, size_t size)
{
	using namespace dds::dxinfo;

	DXGIFormat dxformat = dds::getDDSPixelFormat(data, size);
	bool isSRGB = false;
	bool bgra = false;
	PixelFormat format = convertFormat(dxformat, isSRGB, bgra);

	return validPixelFormat(format);
}

FormatHandler::DecodedImage DDSHandler::decode(const char* data, size_t size)
{
	DecodedImage img;

	dds::Parser parser(data, size);

	bool isSRGB = false;
	bool bgra = false;
	img.format = convertFormat(parser.getFormat(), isSRGB, bgra);

	if (!validPixelFormat(img.format))
		throw Exception("Could not parse DDS pixel data: Unsupported format.");

	if (parser.getMipmapCount() == 0)
		throw Exception("Could not parse DDS pixel data: No readable texture data.");
	...
	try
	{
		img.data = new uint8_t[ddsimg->dataSize];
	}
	catch (std::exception &)
	{
	...
	if (bgra)
	{
		for (int y = 0; y < img.height; y++)
		{
			for (int x = 0; x < img.width; x++)
			{
				...
				uint8_t b = img.data[offset + 0];
				uint8_t r = img.data[offset + 2];
				img.data[offset + 0] = r;
				img.data[offset + 2] = b;
			}
		}
...
bool DDSHandler::canParseCompressed(const char* data, size_t size)
{
	return dds::isCompressedDDS(data, size);
}

CompressedMemory* DDSHandler::parseCompressed(const char* data, size_t size, std::vector<CompressedSlice*> &images, PixelFormat &format, bool &sRGB)
{
	if (!dds::isCompressedDDS(data, size))
		throw Exception("Could not decode compressed data (not a DDS file?)");
	...
	if (texformat == PIXELFORMAT_UNKNOWN)
		throw Exception("Could not parse compressed data: Unsupported format.");

	if (parser.getMipmapCount() == 0)
		throw Exception("Could not parse compressed data: No readable texture data.");
	...
	memory = new CompressedMemory(dataSize);
