...
bool PNGHandler::canDecode(const char* data, size_t size)
{
...
PNGHandler::DecodedImage PNGHandler::decode(const char* data, size_t size)
{
	unsigned int width = 0, height = 0;
	unsigned char *indata = (unsigned char *) data;
	size_t insize = size;
	...
	if (status != 0)
	{
		...
		throw Exception("Could not decode PNG image (%s)", err);
	}
	...
	{
		uint16_t *pixeldata = (uint16_t *) img.data;
		size_t numpixelcomponents = img.size / sizeof(uint16_t);
...
FormatHandler::EncodedImage PNGHandler::encode(const DecodedImage &img, EncodedFormat encodedFormat)
{
	if (!canEncode(img.format, encodedFormat))
		throw Exception("PNG encoder cannot encode to non-PNG format.");
	...
	if (state.info_raw.bitdepth == 16)
	{
		try
		{
			swappeddata = new uint16_t[img.size / sizeof(uint16_t)];
		}
		catch (std::exception &)
		{
		...
		const uint16_t *rawdata = (const uint16_t *) img.data;
		size_t numpixelcomponents = img.size / sizeof(uint16_t);

		for (size_t i = 0; i < numpixelcomponents; i++)
			swappeddata[i] = swapuint16(rawdata[i]);

		data = (const uint8_t *) swappeddata;
	}
#endif
	...
	{
		const char *err = lodepng_error_text(status);
		throw Exception("Could not encode PNG image (%s)", err);
	}
